...
cams=[str(i) for i in range(241,255)]
for i,cam in enumerate(cams):
    files=glob.glob(f'mus/{cam}_TwoPoints_qtr_norm_*.npy')
    filesac=[]
    for j in range(3):
        if j==0:
            filesac.extend( [f'mus/{cam}_TwoPoints_qtr_norm_07{"%02d"%k}.npy' for k in range(dateranges[i][j][0],dateranges[i][j][1]+1)])
        elif j==1:
            filesac.extend( [f'mus/{cam}_TwoPoints_qtr_norm_08{"%02d"%k}.npy' for k in range(dateranges[i][j][0],dateranges[i][j][1]+1)])
        elif j==2:
            filesac.extend([ f'mus/{cam}_TwoPoints_qtr_norm_09{"%02d"%k}.npy' for k in range(dateranges[i][j][0],dateranges[i][j][1]+1)])
    ...
    for file,data in zip(files,datas):
        data=data[5500:8100]
        data=remove_short_act(data,25,20,200)
    ...
    plt.title(f'{cam} 0600-0900 UTC')
    # make xticks labels vertical
    plt.xticks(rotation=90)
    plt.savefig(f'plot/{cam}_bar.pdf')
    plt.figure()
    ...
    # make xticks labels vertical
    plt.xticks(rotation=90)
    plt.savefig(f'plot/{cam}_5bar.pdf')
    ...
    plt.title(f'{cam} 0600-0900 UTC; Date range: {daterangestr}')
    plt.text(0.5,0.5,str(lb))
    
    plt.savefig(f'plot/{cam}_auto.pdf')
plt.show()
